...
if (!totalVisitors) {
totalVisitors = CONFIG.defaultVisitorCount;
...
console.error(“Failed to initialize cursor trail:”, err);
}
}
typeWriterName();
typeWriterBio();
...
if (!isNameDeleting && nameIndex < CONFIG.name.length) {
nameText = CONFIG.name.slice(0, nameIndex + 1);
nameIndex++;
} else if (isNameDeleting && nameIndex > 0) {
nameText = CONFIG.name.slice(0, nameIndex - 1);
nameIndex–;
} else if (nameIndex === CONFIG.name.length) {
isNameDeleting = true;
...
if (!isBioDeleting && bioIndex < CONFIG.bioMessages[bioMessageIndex].length) {
bioText = CONFIG.bioMessages[bioMessageIndex].slice(0, bioIndex + 1);
bioIndex++;
} else if (isBioDeleting && bioIndex > 0) {
bioText = CONFIG.bioMessages[bioMessageIndex].slice(0, bioIndex - 1);
bioIndex–;
} else if (bioIndex === CONFIG.bioMessages[bioMessageIndex].length) {
isBioDeleting = true;
setTimeout(typeWriterBio, 2000);
return;
} else if (bioIndex === 0 && isBioDeleting) {
isBioDeleting = false;
bioMessageIndex = (bioMessageIndex + 1) % CONFIG.bioMessages.length;
}
...
// Glitch on hover profile pic
profilePicture.addEventListener(‘mouseenter’, () => {
glitchOverlay.style.opacity = ‘1’;
setTimeout(() => { glitchOverlay.style.opacity = ‘0’; }, 500);
});

// Fast orbit on click profile pic
...
profilePicture.addEventListener(‘touchstart’, (e) => { e.preventDefault(); triggerFastOrbit(); });
